...
function buildLocalCommands(cli, isLocalProject) {
  console.log("hello novapo medusa here");
  ...
  function resolveLocalCommand(command) {
    ...
    try {
      const cmdPath = resolveCwd.silent(
        `@novapo/medusa/dist/commands/${command}`
      )
  ...
  cli
    ...
    .command({
      command: `start`,
      desc: `Start development server kakakaka.`,
      builder: (_) =>
        _.option(`H`, {
          alias: `host`,
          type: `string`,
...
function isLocalMedusaProject() {
  ...
  try {
    ...
    inMedusaProject =
      (dependencies && dependencies["@novapo/medusa"]) ||
      (devDependencies && devDependencies["@novapo/medusa"])
  } catch (err) {
    /* ignore */
  }
